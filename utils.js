...
exports.saveConfig = function(bot) {
	return new Promise((resolve, reject) => {
		bot.Utils.saveJSON("config.json", bot.Config).then(resolve).catch(reject);
	});
}

exports.savePolls = function(bot) {
	return new Promise((resolve, reject) => {
		bot.Utils.saveJSON("polls.json", bot.custom.polls).then(resolve).catch(reject);
	});
}

exports.saveSlaves = function(bot) {
	return new Promise((resolve, reject) => {
		bot.Utils.saveJSON("slaves.json", bot.custom.slaves).then(resolve).catch(reject);
	});
}
